...
const AppletDir = imports.ui.appletManager.applets['cinnamon7Menu@physics'];
const Boxes = AppletDir.boxes;
...
MyApplet.prototype = {
    ...
    _appletStyles: function (pane) {
        ...
        let searchiconWidth = this._searchInactiveIcon.node_get_theme('icon-size');
	log(searchiconWidth);
        let showTextItemsWidth = this.showTextItems.actor.get_width();
        if (pane == "apps") searchEntryWidth = scrollWidth - searchiconWidth - showTextItemsWidth;
        else searchEntryWidth = favsWidth - searchiconWidth;
        ...
        this.applicationsScrollBox.style = "width: " + ((scrollWidth - showTextItemsWidth) * 0.55) + "px;height: " + scrollBoxHeight + "px;";
        this.categoriesScrollBox.style = "width: " + ((scrollWidth - showTextItemsWidth) * 0.45) + "px;height: " + scrollBoxHeight + "px;";
    },
    ...
    _refreshApps: function () {
        ...
        this._allAppsCategoryButton = new Items.CategoryButton(null);
            ...
            while ((nextType = iter.next()) != GMenu.TreeItemType.INVALID) {
                if (nextType == GMenu.TreeItemType.DIRECTORY) {
                    ...
                    if (this.applicationsByCategory[dir.get_menu_id()].length > 0) {
                        let categoryButton = new Items.CategoryButton(dir);
    ...
    _refreshFavs: function () {
        ...
        let favoritesBox = new Boxes.FavoritesBox();
        ...
        for (let i = 0; i < launchers.length; ++i) {
            ...
            if (app) {
                let button = new Items.FavoritesButton(this, app, launchers.length); // + 3 because we're adding 3 system buttons at the bottom
    ...
    _display: function () {
        ...
        this.placesButton = new Items.PlaceCategoryButton();
    ...
    closeApplicationsContextMenus: function (excludeApp, animate) {
        ...
        for (var fav in this._favoritesButtons) {
            if (fav != excludeApp && this._favoritesButtons[fav].menu.isOpen) {
                if (animate) this._favoritesButtons[fav].toggleMenu();
                else this._favoritesButtons[fav].closeMenu();
            }
        }
    },
    ...
    _displayButtons: function (apps, places) {
        if (apps) {
            for (var i = 0; i < apps.length; i++) {
                ...
                if (!this._applicationsButtons[app]) {
                    let applicationButton = new Items.ApplicationButton(this, app);
        ...
        if (places) {
            for (var i = 0; i < places.length; i++) {
                let place = places[i];
                let button = new Items.PlaceButton(this, place, place.name);
    ...
    _onSearchTextChanged: function (se, prop) {
        ...
        if (this._searchTimeoutId > 0) return;
